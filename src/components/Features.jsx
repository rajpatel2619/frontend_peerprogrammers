...
const Features = () => {
  ...
  const handleHideForm = () => {
    ...
    setTimeout(() => {
      ...
      setFormData({ name: '', poc_name: '', poc_contact_number: '', poc_email: '', address: '', expected_participants: '', message: '' });
    }, 700);
  };

  useEffect(() => {
    if (isFormVisible && rightPaneRef.current) {
      const contentHeight = rightPaneRef.current.scrollHeight;
      // Ensure height is at least the banner height
      setContainerHeight(`${Math.max(contentHeight, 500)}px`);
    } else {
      // Revert to initial banner height
      setContainerHeight('500px');
    }
  }, [isFormVisible, submitStatus]);
  ...
  return (
    <section className="py-20 bg-gray-50 dark:bg-black">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* collage partner banner*/}
        <div className={`bg-gradient-to-br from-black via-gray-900 to-black dark:bg-gradient-to-br dark:from-neutral-800 dark:via-neutral-700 dark:to-neutral-800 text-white border border-gray-700 rounded-2xl mt-20 transition-all duration-500 transform ${isModalOpen ? 'opacity-0 pointer-events-none -translate-y-5' : 'opacity-100 translate-y-0'} shadow-2xl shadow-blue-500/10 hover:shadow-blue-500/20 animate-float`}>
  <div className="max-w-4xl mx-auto text-center py-24 px-4 sm:px-6 lg:px-8">
    <h2 className="text-4xl font-extrabold tracking-tight sm:text-5xl bg-clip-text text-transparent bg-gradient-to-r from-white to-blue-200 animate-pulse-slow">
    ...
    <div className="mt-8">
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-blue-600 to-purple-700 opacity-0 group-hover:opacity-100 transition-opacity duration-300 rounded-md"></div>
      </button>
    </div>
  </div>
</div>
        ...
        <div
          className="mt-20 relative bg-black rounded-2xl overflow-hidden border border-gray-800 transition-all duration-700 ease-in-out"
          style={{ height: containerHeight }}
        >
          {/* Initial Banner View */}
          <div className={`absolute inset-0 transition-all duration-700 ease-in-out ${isFormVisible ? '-translate-x-full opacity-0' : 'translate-x-0 opacity-100'}`}>
            <div className="max-w-4xl mx-auto text-center py-24 px-4 sm:px-6 lg:px-8 flex flex-col items-center justify-center h-full">
          ...
          {/* Inquiry Form View (Split-screen) */}
          <div className={`absolute inset-0 flex transition-all duration-700 ease-in-out ${isFormVisible ? 'translate-x-0' : 'translate-x-full'}`}>
            {/* Left Pane */}
            <div className="w-2/5 bg-black p-12 flex flex-col justify-center text-white">
              <h2 className="text-3xl font-extrabold tracking-tight sm:text-4xl">
                Corporate Inquiry.
              </h2>
              <p className="mt-4 text-lg text-gray-300">
            ...
            <div
              ref={rightPaneRef}
              className="w-3/5 bg-white dark:bg-neutral-900 shadow-2xl p-8 relative"
            >
              <button onClick={handleHideForm} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors">
                <X className="w-6 h-6" />
              </button>
