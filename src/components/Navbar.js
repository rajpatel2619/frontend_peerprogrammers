...
export default function Navbar() {
  ...
  const navBtn = baseBtn + " hover:bg-blue-100/50 dark:hover:bg-blue-900/30 text-gray-700 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400";
  const activeNavBtn = baseBtn + " bg-blue-100/50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400";
  ...
  return (
    <>
      <nav className={`sticky top-0 z-50 bg-white/80 dark:bg-gray-900/80 backdrop-blur-md border-b border-gray-200/50 dark:border-gray-700/50 px-6 md:px-12 lg:px-20 py-3 flex items-center justify-between transition-all duration-300 ${scrolled ? "shadow-sm" : ""}`}>
        {/* Left: Logo */}
        <div 
          className="flex items-center gap-3 cursor-pointer group" 
          onClick={() => navigate("/")}
        >
          <img src={logo} alt="Logo" className="h-10 w-10 transition-transform duration-300 group-hover:scale-110" />
          <span className="hidden md:inline font-extrabold text-xl dark:text-white text-gray-900 select-none bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
            Peer Programmers
          </span>
        </div>
              ...
              {showDropdown && (
                <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-gray-800 rounded-lg shadow-lg z-50 border border-gray-200 dark:border-gray-700 overflow-hidden">
                  ...
                  <div className="py-1">
                    ...
                    <button 
                      onClick={() => { setShowDropdown(false); toggleRole(); }} 
                      className={profileDropdownBtn}
                    >
