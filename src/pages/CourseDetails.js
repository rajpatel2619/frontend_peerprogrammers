...
const CourseDetails = () => {
  ...
  // State for the enrollment modal and form
  ...
  const handlePayment = async () => {
    ...
    try {
      const query = new URLSearchParams({
        user_id: 2, // Assuming a guest or static user ID for now
        course_id: courseId,
        amount: course.price,
      }).toString();
      ...
      const options = {
        ...
        handler: async function (response) {
          setIsModalOpen(false); // Close modal on successful payment initiation
    ...
    <div className="w-full bg-gradient-to-br from-neutral-50 to-neutral-100 dark:from-neutral-950 dark:to-neutral-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          <div className="lg:col-span-3 space-y-8">{/* Skeleton content */}</div>
          <div className="space-y-6">{/* Skeleton sidebar */}</div>
        </div>
      </div>
    </div>
  );
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {course.domain_tags?.map((tag) => (
                      <span key={tag.id} className="px-3 py-1 bg-black/40 text-white rounded-full text-sm font-medium backdrop-blur-sm border border-white/10">
                        {tag.name}
                      </span>
                    ))}
                  </div>
                ...
                {course.syllabus && course.syllabus.length > 0 ? (
                  course.syllabus.map((module, index) => (
                    <div key={index} className="transition hover:bg-neutral-50 dark:hover:bg-neutral-900">
                      <button onClick={() => toggleModule(index)} className="w-full flex justify-between items-center px-6 py-5 text-left focus:outline-none">
              ...
              <div className="divide-y divide-neutral-200 dark:divide-neutral-800">
                {faqs.map((faq, index) => (
                  <div key={index} className="p-6">
                    <button onClick={() => setExpandedFaq(expandedFaq === index ? null : index)} className="flex justify-between items-center w-full text-left">
                      <div className="flex items-center">
            ...
            >
              ...
              <form onSubmit={handleEnrollmentSubmit} className="p-6 space-y-4">
                <div>
                  ...
                  <input type="text" id="name" name="name" value={formData.name} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-transparent focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition" placeholder="John Doe"/>
                </div>
                <div>
                  <label htmlFor="email" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Email Address</label>
                  <input type="email" id="email" name="email" value={formData.email} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-transparent focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition" placeholder="you@example.com"/>
                </div>
                <div>
                  <label htmlFor="phone" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-1">Phone Number</label>
                  <input type="tel" id="phone" name="phone" value={formData.phone} onChange={handleFormChange} className="w-full px-3 py-2 border border-neutral-300 dark:border-neutral-600 rounded-lg bg-transparent focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition" placeholder="9876543210" />
                </div>
